...
const ITEMS = [
  { k: 'notif',  l: 'Notifications', d: 'Gentle nudges. Never spam.',  icon: 'bell',   tone: 'apricot' as const },
  { k: 'health', l: 'Health app',    d: 'Steps + workouts auto-sync.',  icon: 'heart',  tone: 'green' as const },
  { k: 'cam',    l: 'Camera',        d: 'Photo + barcode logging.',     icon: 'camera', tone: 'butter' as const },
  { k: 'mic',    l: 'Microphone',    d: 'Voice logging.',               icon: 'mic',    tone: 'apricotSolid' as const },
];

export default function Permissions({ go }: { go: (r: string) => void }) {
    ...
    <div style={S.page}>
      ...
      <div style={S.pad}>
        ...
        <div style={{ marginTop: 24, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {ITEMS.map(item => {
            const on = done[item.k];
            const iconColor = item.tone === 'apricotSolid' ? '#fff' : C.apricotDk;
            return (
              <div key={item.k} onClick={() => setDone(d => ({ ...d, [item.k]: true }))} style={{ ...S.pillowSm, display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', background: on ? C.greenLt : C.pillow }}>
